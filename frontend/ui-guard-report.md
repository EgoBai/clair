...
### baseline-scan（扫描 599 个文件）

错误: 0 · 警告: 0 · 提示: 1

- **[INFO]** (hardcoded-empty-fallback) 检测到硬编码空兜底 (|| '' / ?? '')，可能掩盖缺失数据。 `./src/pages/BacktestPage.tsx:292`
  - 代码: `formatter={(v) => '¥ ${v ?? ''}'.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`

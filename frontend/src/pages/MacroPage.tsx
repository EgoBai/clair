 ...
 *
 * 后端 /api/macro/* 暂未就绪：接口不可用时如实保持空态，不注入演示数据。
 *
 * 引擎复用：
    ...
    fetch('/api/macro/overview')
      ...
      .then((json) => {
        ...
        if (json?.success && json?.data) {
          setCore(json.data.core ?? []);
          setTrend24(json.data.trend24 ?? []);
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
        <Col>
          <Title level={3} style={{ margin: 0 }}>
            <FundOutlined /> 宏观仪表盘
            {core.length === 0 && <Tag color="default" style={{ marginLeft: 12 }}>数据未接入</Tag>}
          </Title>

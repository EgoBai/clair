      ...
      <div className="discover-container" style={{ maxWidth: 1400, margin: '0 auto', padding: '24px 24px 40px' }}>
        ...
        {view === 'market' ? (
          <>
                        ...
                        <div style={{ display: 'flex', gap: 16, fontSize: 11, color: TEXT_SEC }}>
                          ...
                          {multidimMap[s.industry] && (
                            <div style={{ display: 'flex', gap: 4, marginTop: 4, flexWrap: 'wrap' }}>
                              ...
                              <Tooltip title={`景气度: ${multidimMap[s.industry].boomScore}/100 (扩散+回补+动量仓位+搜索+传播)`}>
                                <span style={{
                                  fontSize: 10, padding: '1px 5px', borderRadius: 3, fontWeight: 700,
                                  background: '#22c55e18', color: '#22c55e',
                                  border: '1px solid #22c55e40',
                                }}>
                                  📈{multidimMap[s.industry].boomScore}
                                </span>
                              </Tooltip>
                              <Tooltip title={`拥挤度: ${multidimMap[s.industry].crowdingScore}/100 (PE分位+集中度+Z值+杠杆+恐慌+基金代理)`}>
                                <span style={{
                                  fontSize: 10, padding: '1px 5px', borderRadius: 3, fontWeight: 700,
                                  background: '#f59e0b18', color: '#f59e0b',
                                  border: '1px solid #f59e0b40',
                                }}>
                                  🔥{multidimMap[s.industry].crowdingScore}
                                </span>
                              </Tooltip>
                            </div>
                          )}</div>
            ...
            {/* ====== 多维景气热力图 v3 (ECharts) — 景气度/拥挤度双重组 ====== */}
            {Object.keys(multidimMap).length > 0 && (() => {
              const sortKey = sortMode === 'boom' ? 'boomScore' : 'crowdingScore';
              ...
              const dimLabels: Record<string, string> = {
                diffusion: '扩散', recovery: '回补', momentumPosition: '动量仓位', searchHeat: '搜索', spreadDegree: '传播',
                crowding: 'PE分位', concentration: '集中度', zScore: 'Z值', leverage: '杠杆', panic: '恐慌', fundFlow: '基金代理',
              };
              ...
              const highlightColIdx = sortMode === 'boom' ? allDimKeys.length : allDimKeys.length + 1;
              ...
              multidimSorted.forEach((item) => {
                allDimKeys.forEach(k => {
                  const dim = (item.dimensions as any)[k];
                  hoverDetail[`${item.industry}__${dimLabels[k]}`] = dim ? dim.label : '';
                });
                hoverDetail[`${item.industry}__景气度`] = `景气度 ${item.boomScore}/100 (扩散+回补+动量仓位+搜索+传播)`;
                hoverDetail[`${item.industry}__拥挤度`] = `拥挤度 ${item.crowdingScore}/100 (PE分位+集中度+Z值+杠杆+恐慌+基金代理)`;
              });
              ...
              const option = {
                tooltip: {
                  ...
                  formatter: (params: any) => {
                    ...
                    const colorHex = isBoom ? '#22c55e' : isSummaryCol ? '#f59e0b' : (val >= 14 ? '#22c55e' : val >= 8 ? '#f59e0b' : '#ef4444');
                    ...
                    data: [[
                      {
                        ...
                        itemStyle: {
                          color: sortMode === 'boom' ? 'rgba(34,197,94,0.08)' : 'rgba(245,158,11,0.08)',
                          borderColor: sortMode === 'boom' ? '#22c55e' : '#f59e0b',
                          borderWidth: 2,
              ...
              const sortLabel = sortMode === 'boom' ? '景气度排序' : '拥挤度排序';
              const sortColor = sortMode === 'boom' ? '#22c55e' : '#f59e0b';
              
              return (
                <div style={{ marginTop: 24 }}>
                  <div style={{ marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <Text strong style={{ color: TEXT, fontSize: 15 }}>🌡️ 多维景气热力 v3</Text>
                      <div style={{ fontSize: 11, color: TEXT_SEC, marginTop: 2 }}>
                        Top10板块 × 11维度(5景气+6拥挤) · 红(0-5)→黄(6-12)→绿(13-20)
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <Button
                        size="small"
                        type={sortMode === 'boom' ? 'primary' : 'default'}
                        onClick={() => setSortMode('boom')}
                        style={{
                          borderRadius: 6,
                          background: sortMode === 'boom' ? '#22c55e' : undefined,
                          borderColor: sortMode === 'boom' ? '#22c55e' : undefined,
                          fontWeight: sortMode === 'boom' ? 700 : 400,
                        }}
                      >
                        📈 景气度排序
                      </Button>
                      <Button
                        size="small"
                        type={sortMode === 'crowding' ? 'primary' : 'default'}
                        onClick={() => setSortMode('crowding')}
                        style={{
                          borderRadius: 6,
                          background: sortMode === 'crowding' ? '#f59e0b' : undefined,
                          borderColor: sortMode === 'crowding' ? '#f59e0b' : undefined,
                          fontWeight: sortMode === 'crowding' ? 700 : 400,
                        }}
                      >
                        🔥 拥挤度排序
                      </Button>

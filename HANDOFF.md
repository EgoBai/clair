...
## 部署哨兵上线记录（2026-09-09 04:10 UTC+8）

### 通道突破：PAT 写权限通道建立
- GitHub 连接器 `ghu_` token 排查定论：纯 OAuth 身份授权（`x-oauth-scopes` 空、`/user/installations`=0、Installed GitHub Apps 无对应 App），权限平台侧写死，**重授权无用**，放弃该通道
- 用户签发 classic PAT（scope: `repo, workflow, write/delete:packages`），已存沙箱 `/root/.codebuddy/.github-pat`（权限600），哨兵探测优先使用
- DNS 修复：`/etc/hosts` 的 `github.com 140.82.112.3` 失效超时 → 换 `140.82.113.3`（备选 114.3 / 116.3 / 121.3 / 20.205.243.166）

### 本次上线内容（deploy 批次收尾，完成上方"遗留"项）
- 推送前核对：远端 main 已前进 7658001 → c678338（12 提交：回测v2全链路 / ui-guard快照同步 / RadarPage lint修复 / 概念端点重写记录）
- 白名单 22 文件逐项核对：9 文件内容已与 deploy 一致、7 个 AI 组件删除已在远端完成；剩余 6 文件差异中 5 个（worker.js/_worker.js 的 -392 行=Backtest v2、RadarPage=Alert lint 回退、HANDOFF/DEV-PLAN=远端记录更全）**均为远端更新版本 → 保留远端版，绝不回退已上线功能**
- **实推仅 1 文件**：`ad2cf47` `ci(collect-history): 上线 collect-history 定时工作流`（cron 30 7 * * 1-5 UTC + workflow_dispatch），fast-forward `c678338..ad2cf47 deploy→main`

### 生产回归（全绿，2026-09-09 04:10）
- `market/summary` 200（avgChangePercent=1.21，真实行情）
- `sectors/concept` 200：**504 个概念** · source=eastmoney · 含真实评分
- `fund-flow/600519` 200：source=eastmoney · 主力净额 + 31 板块流向
- `multidim-v3/batch`[银行] 200：**computed=1** · totalScore=80 · 8 维真实计算

### 收尾
- 项目消息已发（Clair 已上线 + 回归结果）；部署哨兵自动化已 PAUSED（使命完成）
- ⚠️ 后续：push2his 板块K线换源任务仍待立项（见上方复盘）；collect-history cron 触发后 ok 仍会受上游不可达影响；PAT 建议定期轮换，撤销后哨兵退回只读探测态

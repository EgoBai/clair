...
> 最近核对：2026-08-12。原 P0 三项与原 P1 三项均已解决，详见「已解决」小节，勿再按旧标注排期。
...
### 🟢 已解决 — P1 (2026-08-12 核对)

1. ~~策略回测流程断裂(缺日期选择器)~~
   → `BacktestPage.tsx` 已含完整 `RangePicker` + 快捷区间预设 + 区间校验, 起止日期传入 `/api/backtest/run`。
2. ~~产业地图缺少节点下钻(点击→公司列表)~~
   → `IndustryMapPage.tsx` 的 `onNodeClick` 点击节点即于右侧面板 `renderCompanyList` 展示公司列表。
3. ~~投资笔记入口可见性低~~
   → `src/config/navGroups.ts` 已将「投资笔记」从「组合与风控」组末位移至「个股研究」组第 2 位, 紧邻策略选股。
...
## 十一、新增需求与协作约定 (2026-08-12)

### 1. 推送渠道约定 (Requirement: 微信推送渠道)
- **原则**: 若 WorkBuddy 自身移动客户端 / 小程序具备消息通知能力, 则**优先采用 WorkBuddy 自有渠道**实现推送 (告警 / 订单 / 自选异动 / 复盘提醒等), **避免依赖外部方案** (第三方推送网关、企业微信机器人等)。
- **适用触发**: 任何新增「主动推送 / 消息通知」类需求时, 先评估 WorkBuddy 自有渠道可用性; 可用则首选, 不可用才退而求其次外部方案, 并显式记录原因。
- **能力待测**: WorkBuddy 自有推送 API 尚未在本仓库验证就绪 (当前 agent 工具链无对应推送 skill/connector)。实现具体推送功能前, 须先确认 WorkBuddy 移动端 / 小程序的消息下发接口 (订阅消息 / 长连接 / 云函数触发等), 并据此落地。详见 `docs/harness/DESIGN-KB.md` 推送渠道红线。

### 2. 后续协作架构约定 (Requirement: 多 agent 协作)
- **总原则**: 当前已知问题销账闭环后, 后续执行计划按「科学高效的多 agent 协作架构」分工, **合理拆分 + 并行执行**。
- **默认模式**: **蜂群 / 子代理并行 (swarm)** —— 主代理负责规划拆解与编排, 多个专项子代理并行认领独立任务 (后端源接入 / 前端特性 / 测试 / 文档), 借共享任务列表与消息同步进度。
- **灵活选型**: 不固化单一模式。按任务特性选最适配形态 (蜂群并行 / 流水线串行 / 审议式 review); 以当前 agent 开发最佳实践为准, 权衡「任务耦合度 / 上下文隔离 / 冲突风险」。
- **启用前提**: 智能体团队限制开关已解除, 可启用多代理团队 (TeamCreate / Agent 并行派发)。仅在存在**明确可执行计划 + 可独立拆解子任务**时启用, 避免无计划空转。

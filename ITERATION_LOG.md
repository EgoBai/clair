...
### Round 954 (2026-04-25)
- **扩展**: adaptiveStopLossEngine.test.ts — 自适应止损引擎从 6 到 19 个测试
  - 数据不足/30天最低/做多/做空
  - 风险配置: 保守/中等/激进风险排序
  - 结果结构完整性验证
  - 波动率影响(随机)/崩溃行情
  ...
  - 止损类型合理性验证
  - 默认做多方向/short方向
  - computeMultiTimeframeStop: daily+weekly/止损价/不同时间框架/方向参数/风险配置
- 全量测试结果: 1478/1478 文件通过, 32632/32632 测试通过(同一文件), 19/19 ✅
- 轮次状态: 954/1000 (剩余46轮)

### 累计 (Round 954)
- 测试文件: 1478
- 测试数: ~32,632 (+13 tests in same file)
- 全量通过率: **1478/1478 (100%)**, 32632/32632 (100%)

### Round 955 (2026-04-25)
- **扩展**: barraFactorEngine.test.ts — Barra因子引擎从 6 到 14 个测试
  - 空数组/不足5只/正好5只/因子名称验证
  - 结果结构完整性
  - 显著性标记(|tStat|>1.96)
  - 常数暴露因子(零方差)
  - 不同股票数(5/10/50)等量因子
  ...
  - 所有因子暴露有限值
  - 显著性分布合理
- 全量测试结果: 1478/1478 文件通过, 32632/32632 测试通过(同一文件), 14/14 ✅
- 轮次状态: 955/1000 (剩余45轮)
...
### Round 956 (2026-04-25)
- **扩展**: pairTradingEngine.test.ts — 配对交易引擎从 6 到 19 个测试
  - 数据不足返回null
  - 相关配对有效性/发散配对信号
  - 协整得分范围0-1
  - 自定义config覆盖
  ...
  - 同向趋势相关性/平坦序列hurst
  - 不同长度序列以短为准
  - zcScore信号类型/cointegrationScore
  - 纯上升趋势hurst
  - zcScore在发散时超出entry阈值
  - batchPairAnalysis: 多对/空数组/单一配对/多样本排序
- 全量测试结果: 1478/1478 文件通过, 32632/32632 测试通过(同一文件), 19/19 ✅
- 轮次状态: 956/1000 (剩余44轮)
...
### Round 957 (2026-04-25)
- **扩展**: marketSentimentCompositeEngine.test.ts — 市场情绪合成引擎从 7 到 15 个测试
  - 空源/null/单源/多源加权平均
  - 等权重/评级映射/边界测试/结构完整性
  - 零权重/全零权重/负分数/极端分数/大量源/主导性
- 全量测试结果: 1478/1478 文件通过, 32632/32632 测试通过(同一文件), 15/15 ✅
- 轮次状态: 957/1000 (剩余43轮)
...
### Round 958 (2026-04-25)
- **扩展**: backtestEngine.test.ts — 回测引擎从 7 到 14 个测试
  - 空价格/null/无信号
  - 单个买入信号(未平仓)/完整交易→卖出
  - 手续费扣除对比
  - 复杂信号序列/多开多平
...
### 累计 (Round 958)
- 测试文件: 1478
- 测试数: ~32,632 (+7 tests in same file)
- 全量通过率: **1478/1478 (100%)**, 32632/32632 (100%)

...
### 🚫 文档-代码同步红线 (硬约束, 2026-08-12 立)

**自主循环(Hermes/WorkBuddy)关闭 PROJECT-BRIEF §九 任一已知问题(P0/P1/待后端接入)时,
必须在同一轮内同步回写该文档对应条目(标「已解决」+ 证据路径), 禁止只改代码不回写 brief。**

- ❌ 禁止: 关掉一个 issue 后只在 git/PLAN 里记一笔, 留着 PROJECT-BRIEF 里的旧条目当"已知问题"
- ❌ 禁止: 凭文件名/路径臆断"接口不存在/功能缺失"就写入已知问题(P0 曾因 `sector.ts`/`sectors.ts` 笔误误报 404)
- ✅ 正确: 关闭即回写 — 旧条目加删除线 + `证据: 文件:行号`, 顶部核对日期更新为当天
- ✅ 正确: 报"接口/功能不可用"前, 以路由注册表或实际请求验证为准

**背景**: 2026-08-03 与 2026-08-12 两次发现 PROJECT-BRIEF「已知问题」与代码严重漂移
(P0 三项 + P1 三项代码早已实现, 文档却仍标"阻塞/待修复"), 根因是循环关项不回写 brief。
详见 `DEV-COORDINATION.md` 协作约定「关项必回写」。

### 🚫 推送渠道红线 (硬约束, 2026-08-12 立)

新增「主动推送 / 消息通知」类需求时, **优先采用 WorkBuddy 自身移动客户端 / 小程序的消息通知能力**,
避免依赖外部推送方案 (第三方推送网关、企业微信机器人等)。

- ❌ 禁止: 在 WorkBuddy 自有推送渠道可用的情况下, 仍默认引入外部推送依赖
- ✅ 正确: 先评估 WorkBuddy 自有渠道可用性; 可用则首选并据此落地; 不可用才退外部方案, 并显式记录原因
- ⚠️ 能力待测: 当前 agent 工具链无对应推送 skill/connector, 实现前须先确认 WorkBuddy 移动端 / 小程序的
  消息下发接口 (订阅消息 / 长连接 / 云函数触发等)。详见 `PROJECT-BRIEF.md` §十一.1。

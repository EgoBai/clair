...
router.get('/shareholder-changes', validateQuery(schemas.blockTradeQuery), async (req: Request, res: Response) => {
  try {
    ...
    console.error('股东增减持查询失败:', error);
    res.status(500).json({ success: false, error: '股东增减持查询失败' });
  }
});

// 增减持概览（按增减持净额排序）
router.get('/shareholder-changes/overview', async (_req: Request, res: Response) => {
  try {
    ...
    console.error('增减持概览失败:', error);
    res.status(500).json({ success: false, error: '增减持概览失败' });
  }
});
...
router.get('/shareholder-changes/:symbol', validateParams(schemas.stockSymbol), async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      success: true,
      data: { symbol, changes: history, total: history.length },
    });
  } catch (error) {
    console.error('个股增减持查询失败:', error);
    res.status(500).json({ success: false, error: '个股增减持查询失败' });
  }
});
